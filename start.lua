sudo_TiMoR = dofile("sudo.lua")
https = require("ssl.https")
JSON = dofile("./libs/JSON.lua")
local TiMoR_dev = io.open("TiMoR_online.lua")
if TiMoR_dev then
TiMoR_on = {string.match(TiMoR_dev:read('*all'), "^(.*)/(%d+)")}
local tsheke_file = io.open("sudo.lua", 'w')
tsheke_file:write("token = '" ..TiMoR_on[1].."'\n\nsudo_add = "..TiMoR_on[2].."" )
tsheke_file:close()
https.request("https://api.telegram.org/bot"..TiMoR_on[1].."/sendMessage?chat_id="..TiMoR_on[2].."&text=Bot_TiMoR_is_start_new")
os.execute('cd .. && rm -rf .telegram-cli')
os.execute('rm -rf TiMoR_online.lua')  
os.execute('./tg -s ./TiMoR.lua $@ --bot='..TiMoR_on[1])
end
...
os.execute('cd && rm -fr .telegram-cli')
os.execute('./tg -s ./TiMoR.lua $@ --bot='..tokenCk)
else
...
os.execute('wget https://raw.githubusercontent.com/FAYNLMAX/TiMoR/master/TiMoR.lua')
dofile('./TiMoR.lua')  
os.execute('./tg -s ./TiMoR.lua $@ --bot='..token)
else
